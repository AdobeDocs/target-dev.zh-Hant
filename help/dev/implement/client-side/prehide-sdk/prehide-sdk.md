...
></br>>請勿將`async`或`defer`新增至預先隱藏SDK指令碼標籤。 需要同步執行，以便在瀏覽器開始佈局頁面之前插入隱藏規則。
...
當您從Flicker Manager UI下載`prehide.min.js`時，伺服器會取代套件中的三個預留位置：

| 預留位置 | 已取代為 | 如果未取代則遞補 |
| --- | --- | --- |
| `__FM_CLIENT_CODE__` | 您的使用者端代碼（例如，`"acmecorp"`） | 讀取`window.PrehideConfig.org` |
| `__FM_TIMEOUT__` | 保護計時器持續時間（以毫秒為單位，例如`"3000"`） | `5000`毫秒 |
| `__FM_VERSION__` | SDK版本（例如，`"1.0.0"`） | `"0.0.0-dev"` |

如果您使用UI下載的套件組合，則不需要使用`PrehideConfig`區塊。 只需內嵌指令碼。

### B：執行階段`window.PrehideConfig` （手動整合）
...
| 欄位 | 類型 | 必要 | 說明 |
| --- | --- | --- | --- |
| `org` | string | 是（除非已存回） | 您的客戶使用者端代碼。 當做擷取預先隱藏規則的CDN URL的組織區段。 |
