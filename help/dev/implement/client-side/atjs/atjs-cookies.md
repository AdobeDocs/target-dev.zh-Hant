...
description: 有關如何操作的詳細資訊 [!DNL Adobe Target] at.js 2.x和at.js 1.x處理Cookie
title: at.js Cookies
feature: at.js
...
若為at.js 2.x版(最高至（但不包括） 2.10.0版)， *僅支援第一方Cookie*. 就像使用 at.js 1.*x*，第一方Cookie「mbox」會儲存在 `clientdomain.com`，其中 `clientdomain` 是您的網域。

at.js 會產生一個工作階段 ID 並將其儲存於 Cookie 中。第一個回應包含任何活動資訊，以及 [!DNL Target] 伺服器產生的 `TNT` 或 `PC ID`。然後 at.js 會將 `TNT/PC ID` 寫入 Cookie 中。

此 `AMCV_###@AdobeOrg` 第一方Cookie一律由Experience CloudID服務設定，不過 `ECID` 已傳入 [!DNL Target] 要求。
...
您的網站設定會決定您要使用何種 Cookie。此資訊有助於瞭解如何 [!DNL Target] 在嘗試瞭解第一方和第三方Cookie時有效。 另請參閱 [如何 [!DNL Adobe Target] 工作](https://experienceleague.adobe.com/docs/target/using/introduction/how-target-works.html) 以取得詳細資訊。
...
>[!NOTE]
>
>此 `AMCV_###@AdobeOrg` 第一方Cookie一律以Experience Cloud訪客ID設定。
...
| Cookie 持續時間 | Cookie會保留在訪客的瀏覽器上達兩年時間，期限從訪客最後登入算起。<P>此 `deviceIdLifetime` 設定在中可覆寫 [at.js 2.3.1版或更新版本](../atjs/target-atjs-versions.md). 如需詳細資訊，請參閱 [targetGlobalSettings()](../../../implement/client-side/atjs/atjs-functions/targetglobalsettings.md)。 |
...
## 影響 [!DNL Target] 若為Safari訪客，由於Apple WebKit追蹤變更

請記住下列事項:

### 如何 [!DNL Adobe Target] 追蹤工作？

| Cookie | 詳細資料 |
|--- |--- |
| 第一方網域 | 此為的標準實作 [!DNL Target] 客戶。  「mbox」Cookie 設定在客戶的網域中。 |
| 第三方追蹤 | 第三方追蹤對於中的廣告和鎖定使用案例很重要 [!DNL Target] 和 [!DNL Adobe Audience Manager] (AAM)。  第三方追蹤需要跨網站指令碼技術。[!DNL Target] 使用 `clientcode.tt.omtrd.net` 網域中設定的兩個 Cookie:「mboxSession」和「mboxPC」。 |
...
| 使用「機器學習」來辨別跨網站的網域 | 從 Apple: <P>機器學習分類器: 機器學習模型用來分類哪些私人控制的最上層網域有能力根據收集的統計資料，而跨網站追蹤使用者。從各種收集的統計資料中，有三個向量出現強烈訊號，表示應該根據目前的追蹤實務來分類: 一些唯一網域下的子資源、一些唯一網域下的子範圍，以及重新導向到的一些唯一網域。資料收集和分類全都在裝置上進行。<P>不過，如果使用者與當作最上層網域的 example.com (通常稱為第一方網域) 互動，「智慧型追蹤預防」會將此視為訊號，認為使用者對此網站有興趣，並依此時間表來暫時調整其行為:<P>如果使用者在過去 24 小時與 互動，則其 Cookie 在 `example.com`exampl .com 是第三方時就可用。這允許「在 Y 上以我的 X 帳戶登入」登入情節。<ul><li>當作最上層網域來造訪的網域不受影響。例如，OKTA 網站</li><li>跨多個唯一網域來識別作為目前頁面之子網域或子範圍的網域。</li></ul> |

### 對 Adob 有何影響?

| 受影響的功能 | 詳細資料 |
|--- |--- |
| 支援選擇退出 | Apple 的 WebKit 追蹤變更會暫停選擇退出支援。<P>[!DNL Target] 選擇退出會在 `clientcode.tt.omtrdc.net` 網域中使用 Cookie。如需更多詳細資料，請參閱[隱私](/help/dev/before-implement/privacy/privacy.md)。<P>[!DNL Target] 支援兩種選擇退出：<ul><li>由用戶端決定 (用戶端管理選擇退出連結)。</li><li>一個透過Adobe選擇使用者退出所有 [!DNL Target] 功能，適合所有客戶。</li></ul>兩種方法都使用第三方 Cookie。 |
| [!DNL Target] 活動 | 客戶可選擇他們的 [設定檔存留期長度](https://experienceleague.adobe.com/docs/target/using/audiences/visitor-profiles/visitor-profile-lifetime.html) (針對其 [!DNL Target] 帳戶 — 最長90天。 問題是如果帳戶的設定檔存留期超過30天，且第一方Cookie因為客戶的網域已標示為跨網站追蹤而被清除，則Safari訪客的行為將在的以下區域中受到影響 [!DNL Target]：<P>**[!DNL Target]報表**：如果Safari使用者進入活動，30天後回訪並轉換，則該使用者會計為兩個訪客和一次轉換。<P>對於使用的活動，此行為是相同的 [!DNL Analytics] 做為報表來源(A4T)。<P>**設定檔與活動成員資格**:<ul><li>第一方 Cookie 到期時會清除設定檔資料。</li><li>第一方 Cookie 到期時會清除活動成員資格。</li><li> [!DNL Target]對於採用第三方 Cookie 實作或第一方和第三方 Cookie 實作的帳戶， 在 Safari 中沒有作用。請注意，這不是新的行為。Safari 暫時還不允許第三方 Cookie。</li></ul><P>**建議**：如果擔心客戶網域可能標示為跨工作階段追蹤訪客，最安全的做法是將設定檔存留期設定為30天或更少 [!DNL Target]. 這樣可確保在 Safari 和所有其他瀏覽器中以類似方式追蹤使用者。 |
