...
1. 如果已啟用全域mbox自動建立，[!DNL Target] JavaScript程式庫：
   ...
   * 實體化訪客物件。
   * [!DNL Target]資料庫會嘗試擷取Experience Cloud訪客ID資料。
...
>[!NOTE]
>
>對於新訪客，當訪客API出現時，[!DNL Target]必須多次透過線路以確保[!DNL Target]要求包含Experience Cloud訪客ID資料。 若為回訪訪客，[!DNL Target]僅會透過線路傳送至[!DNL Target]以擷取個人化內容。
...
如果您是透過非同步[!UICONTROL Adobe Experience Platform]實作部署at.js，請務必在使用[!UICONTROL Adobe Experience Platform]內嵌程式碼實作[!DNL Target]之前，直接在頁面上包含預先隱藏的程式碼片段。
...
at.js實作使用單一資料庫(at.js)，而mbox.js實作實際上使用兩個資料庫（mbox.js和target.js）。 所以，at.js 要同時與 mbox.js *和* `target.js` 一起比較才公平。 at.js 1.2版和mbox.js 63版的gzipped大小比較皆為34 KB，則為26.2 KB。 &quot;
...
## 我可以在單頁應用程式中使用Target [!UICONTROL 視覺化體驗撰寫器] (VEC)嗎？

可以，若您使用at.js 2.x，便可針對SPA使用VEC。 如需詳細資訊，請參閱[單一頁面(SPA)視覺化體驗撰寫器](https://experienceleague.adobe.com/docs/target/using/experiences/spa-visual-experience-composer.html?lang=zh-Hant)。
...
tt.omtrdc.net是Adobe EDGE網路的網域名稱，用來接收[!DNL Target]的所有伺服器呼叫。
...
只要targetGlobalSettings()函式([targetGlobalSettings](/help/dev/implement/client-side/atjs/atjs-functions/targetglobalsettings.md))中的`secureOnly`選項設為true，與at.js所啟用Adobe Edge網路的通訊就會透過HTTPS進行，否則at.js會根據頁面通訊協定，允許在HTTP與HTTPS之間切換。
...
此外，向Adobe Edge Network發出的請求是公開的（因為這些請求是設計成從訪客的瀏覽器中發出），並且不包含可見的訪客詳細資料（只包含訪客ID）。 這些請求會將體驗傳送給訪客，而且包含訪客應在頁面上看到的詳細資訊。
...
在at.js要求中，應該會看到值為「*」的`Access-Control-Allow-Origin`標頭，因為它們是公開的，不需要驗證，並且需要透過JavaScript呼叫從任何網域存取Adobe Edge Network。
...
at.js會試圖長時間避免預先隱藏HTML BODY或其他DOM元素，但這取決於網路條件和活動設定。 at.js 提供的[設定](/help/dev/implement/client-side/atjs/atjs-functions/targetglobalsettings.md)可用於自訂隱藏 CSS 樣式的 BODY 設定，如此一來您就可以僅預先隱藏頁面的某些部份，而不用隱藏整個 HTML BODY。 期望是那些部分包含了必須「個人化」的 DOM 元素。
