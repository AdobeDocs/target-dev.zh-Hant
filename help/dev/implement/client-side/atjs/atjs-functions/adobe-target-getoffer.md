...
description: 使用 [!UICONTROL adobe.target.getOffer()] 函式及其選項 [!DNL Adobe Target] at.js程式庫觸發要求，以取得 [!DNL Target] 選件。
...
| timeout | 數字 | 無 | 逾時，以毫秒為單位。如果未指定，將使用 at.js 中的預設逾時。<P>預設逾時可由以下設定： [!DNL Target] UI在 [!UICONTROL 管理] > [!UICONTROL 實施]. |

## 範例

新增引數，使用 [!UICONTROL getOffer()] 和使用 [!UICONTROL applyOffer()] 若為成功處理：
...
新增引數和設定檔引數，使用 [!UICONTROL getOffer()] 和使用 [!UICONTROL applyOffer()] 若為成功處理：
...
搭配使用自訂逾時和自訂成功處理 [!UICONTROL getOffer()]：
