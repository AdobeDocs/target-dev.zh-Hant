...
| 8 | 目標資料透過SDID符合[!UICONTROL Analytics]資料，且已處理至[!UICONTROL Analytics]報表儲存體。<P>然後就可以在[!UICONTROL Analytics]與[!UICONTROL Analytics]中，透過[!DNL Target] (A4T)報表來檢視[!UICONTROL Analytics for Target]資料。 |
...
| 6 | [!DNL Target]資料透過SDID與[!UICONTROL Analytics]資料相符，並且已處理至[!UICONTROL Analytics]報表儲存體。 然後就可以透過A4T報表在[!UICONTROL Analytics]和[!UICONTROL Analytics]中檢視[!DNL Target]資料。 |

## 部署 at.js 2.*x*

部署 at.js 2.透過&#x200B;*Adobe Experience Platform*&#x200B;擴充功能中的標籤[x](/help/dev/implement/client-side/atjs/how-to-deployatjs/implement-target-using-adobe-launch.md)。
...
>或
>
>手動下載at.js 2.使用&#x200B;*UI的* x[!DNL Target]，並使用您選擇的[方法](/help/dev/implement/client-side/atjs/how-to-deployatjs/how-to-deployatjs.md)進行部署。
...
在 at.js 2.*x*，網路呼叫中不會再出現「`target-global-mbox`」。 我們已改為將傳送至`target-global-mbox`伺服器的JSON裝載中的&quot;`execute > pageLoad`&quot;語法取代為&quot;[!DNL Target]&quot;，如下所示：
...
客戶可以透過&#x200B;**[!UICONTROL Target]** > **[!UICONTROL Administration]** > **[!UICONTROL Implementation]** > **[!UICONTROL Edit at.js Settings]**&#x200B;指定全域mbox名稱。 此設定供 [!DNL Target] Edge 伺服器用來將 execute > pageLoad 轉譯為 [!DNL Target] UI 中顯示的全域 mbox 名稱。這麼做可讓客戶繼續使用伺服器端 API、表單式撰寫器、輪廓指令碼，以及使用全域 mbox 名稱建立客群。強烈建議您確認也已在&#x200B;**[!UICONTROL Administration]** > **[!UICONTROL Visual Experience Composer]**&#x200B;頁面上設定相同的全域mbox名稱(若您仍有頁面使用at.js 1.*x*，如下列圖例所示。
...
跨網域追蹤能夠拼接不同網域之間的訪客。由於必須為各個網域建立新 Cookie，因此在訪客從某個網域導覽至另一個網域時，就很難加以追蹤。為實現跨網域追蹤，[!DNL Target] 使用第三方 Cookie 來追蹤網域之間的訪客。這可讓您建立橫跨[!DNL Target]和`siteA.com`的`siteB.com`活動，而且當訪客在各個唯一網域導覽時，訪客會維持相同的體驗。 此功能與[!DNL Target]的第三方和第一方Cookie行為整合。
...
不過，在at.js 2.未使用&#x200B;*x*，HTTP GET。 而是透過at.js 2.*x*&#x200B;將JSON裝載傳送至[!DNL Target]部Edge伺服器。 使用HTTP POST表示檢查瀏覽器是否支援第三方Cookie的重新導向要求將中斷。 這是因為 HTTP GET 要求為等冪交易，而 HTTP POST 是非等冪交易且不得任意重複。因此，at.js 2.*x* （2.10之前）不支援立即可用。 只有 at.js 1.*x* 才提供跨網域追蹤的立即可用支援。
...
1. 如果您有at.js v2.10或更新版本，您不必安裝ECID程式庫，而是可以在[!DNL Target] > **[!UICONTROL Administration]**&#x200B;的&#x200B;**[!UICONTROL Implementation]** UI中啟用跨網域設定。 （或者，您可以在at.js程式碼中將&#x200B;_crossDomain_&#x200B;選項設為&#x200B;_enabled_。）
...
### 它表示當我使用&amp;amp；lbrace；`triggerView()`&amp;amp；rbrace；呼叫`"page" : "true"`時，它會傳送通知至[!DNL Target]後端並增加曝光次數。 那麼也會造成輪廓指令碼執行嗎?
...
>[!NOTE]
>
>透過at.js 2.[!UICONTROL Auto-Target]當所有修改皆已套用至&#x200B;*時，* x`Page Load Event`和VEC。 將修改新增至特定檢視時，僅支援[!UICONTROL A/B Test]、[!UICONTROL Auto-Allocate]和[!UICONTROL Experience Targeting] (XT)活動。
