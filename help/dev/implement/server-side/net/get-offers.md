---
title: 使用.NET SDK時，請在 [!DNL Adobe Target] 中使用getOffers()
...
`TargetDeliveryRequest`已使用`TargetDeliveryRequest.Builder`建立。
...
`TargetDeliveryRequest.Builder`物件具有下列結構：
...
| 名稱 | string | Cookie 名稱 |
| 值 | string | Cookie值 |
| MaxAge | int | `MaxAge`選項可方便您設定相對於目前時間（以秒為單位）的「過期」 |

您不必擔心Cookie會過期。 SDK內的[!DNL Target]處理`MaxAge`。
