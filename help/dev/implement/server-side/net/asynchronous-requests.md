---
title: 如何在中使用非同步請求 [!DNL Adobe Target] .NET SDK
...
伺服器端整合的好處之一，就是可以利用伺服器端可用的大量頻寬和運算資源，透過使用平行功能。 [!DNL Target] .NET SDK支援非同步請求，可輕鬆整合 [!DNL Target] 並放入應用程式現有的非同步工作流程中。
...
此範例假設您擁有 [已初始化SDK](initialize-sdk.md).
