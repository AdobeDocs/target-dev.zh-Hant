...
| requestInterceptor | HttpRequestInterceptor | 否 | Null | 新增自訂請求攔截器 |
| defaultPropertyToken | 字串 | 無 | 無 | 設定每個`getOffers`呼叫的預設屬性權杖。 **針對裝置上決策**，SDK將只會下載包含在`defaultPropertyToken`中設定的屬性權杖的合格活動的成品 |
...
| proxyConfig | ClientProxyConfig | 無 | 無 | 允許使用者端提供自己的Proxy詳細資料 |
| exceptionHandler | TargetExceptionHandler | 無 | 無 | 可用於在規則處理期間實作自訂例外狀況處理 |
| httpClient | HttpClient | 無 | 無 | 允許使用者以自訂HTTP使用者端取代[!DNL Target] HTTP使用者端 |
| onDeviceEnvironment | 字串 | 否 | 生產 | 可用來指定不同的裝置上環境，例如測試 |
...
| onDeviceArtifactPayload | 位元組[] | 無 | 無 | 提供裝置上決策，搭配先前的成品裝載，以便立即執行 |
| Devicedecisioninghandler | OnDeviceDecisioningHandler | 無 | 無 | 註冊裝置上決策事件的回呼 |
| onDeviceAllMatchingRulesMboxes | 清單\&lt;字串\> | 無 | 無 | 允許使用者指定在裝置上決策期間將傳回所有相符規則內容的mbox |
