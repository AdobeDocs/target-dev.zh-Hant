...
description: 瞭解如何使用 [!DNL Adobe Target] 動態測試電子郵件中的影像，甚至當某人開啟電子郵件時立即變更這些影像。
...
>[!NOTE]
>
>雖然[!DNL Target]技術上可在開啟時最佳化影像，但每個電子郵件使用者端處理快取的方式不同，因此成功情況也有所不同。 不論使用的電子郵件服務提供者為何，使用者用來開啟電子郵件的電子郵件用戶端 (Gmail 應用程式、Outlook、Hotmail 等) 會決定實際上是否能在開啟時擷取影像。 舉例來說，Gmail 會快取大部分內容，因此使用者看到的影像是根據 Gmail 何時選擇呼叫並快取影像。
...
| 值 | 說明 |
|--- |--- |
| clientcode | 公司的用戶端代碼。 在at.js中找出列為`clientCode='yourclientcode'`的此專案。 全部都是小寫，不包含特殊字元。 |
| 影像 | 產品建議類型。 對於影像廣告一律為「image」，而對於重新導向程式則為「page」。 |
...
| `mboxXDomain=disabled` | 告知[!DNL Target]不要嘗試設定Cookie。 |
