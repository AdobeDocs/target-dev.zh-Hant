...
智慧型追蹤預防(ITP)是Apple保護Safari使用者隱私權的計畫。 ITP 是在 2017 年首次發行，將目標鎖定於第三方 Cookie 的使用方式。 事實上，Apple 完全封鎖了第三方 Cookie，進而對廣告技術和行銷技術公司造成了嚴重的問題，因為第三方 Cookie 通常是用來追蹤訪客及收集訪客資料。 現在，Apple 正在努力朝向針對 Safari 內第一方 Cookie 的使用方式設定限制。
...
| [ITP 2.3](https://webkit.org/blog/9521/intelligent-tracking-prevention-2-3/) | 已消除數個因應措施，例如採用localStorage或使用JavaScript `Document.referrer property`。<br />於2019年9月23日發行。<br />在Safari 14、macOS Big Sur、Catalina、Mojave、iOS 14和iPadOS 14中發行的ITP的CNAME遮蔽防禦功能。 由第三方CNAME遮罩的HTTP回應建立的所有Cookie都會設定為在7天後過期。<br />於2020年11月12日宣佈。 |

## 身為[!DNL Target]客戶的這對我有何影響？

Target提供JavaScript資料庫供您在頁面上部署，這樣[!DNL Target]就可以向訪客提供即時個人化。 有三個[!DNL Target]個JavaScript程式庫at.js 1.*x*、at.js 2.*x*，這個[!DNL Adobe Experience Cloud Web SDK]可透過`document.cookie` API將使用者端[!DNL Target] Cookie放在您訪客的瀏覽器上。 因此，[!DNL Target] Cookie會受到Apple ITP 2.1、2.2和2.3的影響，且將在七天後過期（若是ITP 2.1）以及在一天後過期（若是ITP 2.2和ITP 2.3）。
