...
Target提供JavaScript資料庫供您在頁面上部署，這樣[!DNL Target]就可以向訪客提供即時個人化。 有三個[!DNL Target]個JavaScript資料庫at.js 1.*x*， at.js 2.*x*，透過[!DNL Adobe Experience Cloud Web SDK] API將使用者端[!DNL Target] Cookie放在訪客瀏覽器上的`document.cookie`。 因此，[!DNL Target] Cookie會受到Apple ITP 2.1、2.2和2.3的影響，且將在七天後過期（若是ITP 2.1）以及在一天後過期（若是ITP 2.2和ITP 2.3）。
