...
* 如果您使用[!UICONTROL Bulk Profile Update API] v2，則不需要為每個`pcId`指定所有引數值。 已為[!DNL Target]中找不到的任何`pcId`或`mbox3rdPartyId`建立設定檔。 如果您使用v1，則不會為遺失的pcIds或mbox3rdPartyIds建立設定檔。 如需詳細資訊，請參閱下列 [!DNL Bulk Profile Update API]](#empty)中的[處理空白值。
...
此行為同時適用於[!DNL Bulk Profile Update API]的v1和v2。
